...
function Home() {
  ...
  function addToReadingList(book) {
    ...
    localStorage.setItem("readingList", readingList);

    // Remove book from original list
    books = books.filter((element) => element.book.title !== book.title);
    setBooks(books);
    localStorage.setItem("books", books);
  }

  function deleteFromReadingList(book) {
    // Add new element
    books.push(book);
    setBooks(books);
    localStorage.setItem("books", books);
    ...
    // Remove book from original list
    readingList = readingList.filter((element) => element.title !== book.title);
    setReadingList(readingList);
    localStorage.setItem("readingList", readingList);
  }
  ...
  return (
    <div className="Home">
      ...
      <div className='div-books'>
        Cantidad de libros disponibles ({books.length})
      </div>
      {books.map(function (element) {
        return (
          <div key={element.book.title}>
            <div>
              Título: {element.book.title} | Genero: {element.book.genre} | <button onClick={() => addToReadingList(element.book)}>   Agregar a lista de lectura</button>
            </div>
            <br></br>
      ...
      {readingList.map(function (element) {
        return (
          <div key={element.title}>
            <div>
              Título: {element.title} | Genero: {element.genre} | <button onClick={() => deleteFromReadingList(element)}>   Eliminar de lista de lectura</button>
            </div>
            <br></br>
